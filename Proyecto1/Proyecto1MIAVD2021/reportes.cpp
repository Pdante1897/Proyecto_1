            ...
            if(!banderaName){
                if(!banderaId){
                    ...
                    if(particion.id != "nulo" ){
                        ...
                        if(valorName == "mbr"){
                            Graficar(1,particion.dir,"", valorPath);
                        }
                        else if(valorName == "disk"){
                            Graficar(2,particion.dir,"", valorPath);
                        }else if(valorName == "superbloque"){
                            Graficar(3,particion.dir, particion.name,valorPath);
                        }else if(valorName == "bm_inode"){
                            Graficar(4,particion.dir, particion.name,valorPath);
                        }else if(valorName == "bm_block"){
                            Graficar(5,particion.dir, particion.name,valorPath);
                        }else if(valorName == "journal"){
                            //Graficar_journal(particion.dir, particion.name,valorPath);
                        }else if(valorName == "inode"){
                            Graficar(6,particion.dir, particion.name,valorPath);
                        }else if(valorName == "block"){
                            Graficar(7,particion.dir, particion.name,valorPath);
                        }
                    }else
...
void Graficar(int reporte,QString path, QString particion, QString pathDest){
    ...
    string comandoGraficar = "dot -Tpng /home/bryan/blocks.txt -o " + pathDest.toStdString();
    FILE *archivo;
    if((archivo = fopen(path.toStdString().c_str(), "rb+"))){
    }else{
        printf(ANSI_COLOR_CYAN"Error: No se existe el disco deseado \n" ANSI_COLOR_RESET);
